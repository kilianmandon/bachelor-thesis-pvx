\documentclass[12pt]{article}
\usepackage[english]{babel}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{textcomp}
\usepackage{lmodern}
\usepackage{graphicx}
\usepackage[hidelinks]{hyperref}
\usepackage[a4paper, lmargin={4cm}, rmargin={2cm}, tmargin={2.5cm}, bmargin={2.5cm}]{geometry}
\usepackage{enumerate}
\usepackage{listings}
\usepackage{amssymb}
\usepackage{array,ragged2e}
\usepackage{svg}
\usepackage{makecell}
\usepackage{amsthm}
\usepackage{amsmath}
\usepackage{tabularx}
\usepackage{longtable}
\usepackage{booktabs}
\usepackage{faktor}
\usepackage{dsfont}
\usepackage{bm}
\usepackage[section]{placeins}
\usepackage{nicefrac}
\usepackage{siunitx}
\usepackage{tabularray}
\usepackage{caption}

\usepackage{lightalgostyle}

\usepackage{newfloat}

\DeclareFloatingEnvironment[
    fileext=losf,
    listname=List of Supplementary Figures,
    name=Supplementary Figure,
    placement=htbp,
    within=none,
]{suppfigure}

\DeclareFloatingEnvironment[
    fileext=lost,
    listname=List of Supplementary Tables,
    name=Supplementary Table,
    placement=htbp,
    within=none,
]{supptable}

\DeclareFloatingEnvironment[
    fileext=losa,
    listname=List of Supplementary Algorithms,
    name=Supplementary Algorithm,
    placement=htbp,
    within=none,
]{suppalgorithm}

% Set the counter format to use S prefix
\renewcommand{\thesuppfigure}{S\arabic{suppfigure}}
\renewcommand{\thesupptable}{S\arabic{supptable}}
\renewcommand{\thesuppalgorithm}{S\arabic{suppalgorithm}}

% Apply the ruled float style to match algorithm package
\floatstyle{ruled}
\restylefloat{suppalgorithm}

\newcommand{\algorithmautorefname}{Algorithm}

\captionsetup{labelfont=bf,tableposition=top}

\graphicspath{ {images/} }

\usepackage{csquotes}
\usepackage[sorting=none]{biblatex}
\addbibresource{chapters/bachelor_sources.bib}


\DeclareSIUnit\angstrom{\text {Å}}
\DeclareSIUnit\Dalton{\text {Da}}
\DeclareSIUnit\Molar{M}
\DeclareSIUnit\enzymeUnit{U}
\DeclareSIUnit\cal{cal}


\begin{document}
\begin{titlepage}
    \begin{center}
         \vspace*{0.5cm}
         
         \textbf{\underline{Bachelor's Thesis}}
         
         \vspace{1.5cm}
         
         \large
         Submitted in Fulfilment of the Requirements for the Degree of\\
         Bachelor of Science (B. Sc.) in Molecular and Applied Biotechnology\\
         RWTH Aachen University
         
         \vspace{1cm}
         
         \rule{\textwidth}{0.4pt}
         
         \vspace{1cm}
         
         \textbf{\LARGE Computational Design and Cell-Free Expression of
         Coat Protein Subunits for the Functional Assembly into
         Potato Virus X-like Particles}
         
         \vspace{1cm}
         
         \rule{\textwidth}{0.4pt}
         
         \vspace{1cm}
         
         Faculty of Mathematics, Computer Science and Natural Sciences\\
         RWTH Aachen University\\
         
         At the Institute for Molecular Biotechnology
         
         \vspace{1cm}
         
         \begin{tabular}{ll}
         \textbf{Submitted by} & Kilian Mandon \\
         \textbf{Supervisor} & Dr. rer. nat. Juliane Schuphan \\
         \textbf{First examiner} & Prof. Dr. Stefan Schillberg \\
         \textbf{Second examiner} & Jun. Prof. Dr. Anna Matuszyńska \\
         \end{tabular}
         
         \vspace{1cm}
         
         Aachen, \today
         
    \end{center}
 \end{titlepage}

\tableofcontents
\clearpage

\section{Introduction}\label{ch:introduction}
\input{chapters/01_introduction}
\FloatBarrier

% \section{Notation}\label{ch:notation}
% \input{chapters/02_notation}
% \FloatBarrier
\clearpage
\part{Computational Modeling}
The computational design pipeline employed in this project consists of four steps for creation and in silico assessment of the designs. The designs are created using RFdiffusion and ProteinMPNN, where RFdiffusion creates backbones for the PVX symmetry, and ProteinMPNN realizes those backbones with amino acid sequences. In addition to usage on RFdiffusion backbones, ProteinMPNN is also used to design novel sequences based on the wild type structure of the PVX CP. Analysis of the designs is conducted using AlphaFold and GROMACS. The structure prediction tool AlphaFold is used to judge designed sequences based on whether the predicted fold agrees with the structure the design was created for, while the molecular dynamics software GROMACS analyzes the structures regarding their stability and binding energy (\autoref{fig:computational_overview}). 

\begin{figure}[H]
\includegraphics[width=\textwidth]{computational_overview.png}
\caption{\textbf{Overview of the computational design pipeline.} The AI-based tools RFdiffusion and ProteinMPNN are used for backbone and sequence design. The structure prediction tool AlphaFold analyzes and selects designs based on their predicted fold. The molecular dynamics software GROMACS further evaluates the design's stability and binding energy. }
\label{fig:computational_overview}
\end{figure}

\section{Protein Design}
\subsection{Symmetry Analysis and Model Building}\label{ch:symmetry}
\input{chapters/03_symmetry}
\FloatBarrier

\subsection{Sequence Design with ProteinMPNN}\label{ch:pmpnn}
\input{chapters/04_pmpnn}
\FloatBarrier

\subsection{Backbone Design with RFdiffusion}\label{ch:rfdiffusion}
\input{chapters/05_rfdiffusion}
\FloatBarrier

\section{Computational Validation}\label{ch:design_evaluation}
\subsection{Evaluation with AlphaFold}\label{ch:alphafold}
\input{chapters/06_alphafold}
\FloatBarrier

\subsection{Evaluation with GROMACS}\label{ch:gromacs}
\input{chapters/07_gromacs}
\FloatBarrier

\clearpage
\part{Experimental Evaluation}
\section{Materials}\label{ch:materials}
\input{chapters/08_materials}

\section{Methods}\label{ch:methods}
\input{chapters/09_methods}

\section{Results}\label{ch:results}
\input{chapters/10_results}

\section{Discussion}\label{ch:discussion}
\input{chapters/11_discussion}

\section{Conclusion}\label{ch:conclusion}
\input{chapters/12_conclusion}

\clearpage
\printbibliography

\clearpage
\input{chapters/appendix.tex}

\clearpage
\input{chapters/acknowledgements.tex}

\end{document}