The structure of PVX was determined by Grinzato et al. (2020) \cite{Grinzato2020}, up to a resolution of $\SI{2.2}{\angstrom}$. The structural data was made available through the PDB, as a file containing $13$ consecutive protein subunits, forming one-and-a-half cycles of the helix. Notably, structural determination was not possible for the 28 amino acids long N-terminal domain, due to its flexibility. The N-terminal domain is relevant for the particle's structure and assembly. In experiments by Betti et al. (2011) \cite{del22_rigid}, it was found that while a deletion of the 22 N-terminal amino acids still leaves the virus infectious, the morphology of the particles changes from the wild type. The structure-guided computational design process will only be used to design the rigid part of the protein, the N-terminal domain will however be fused to the constructs in the end. 

The following chapters require a flexible way to use the symmetry of PVX, such as the ability to generate different configurations of monomers (e.g. a 3x3 neighborhood of monomers), or the ability to dynamically enforce this symmetry during symmetry-guided design with RFdiffusion (\autoref{ch:rfdiffusion}) or symmetry-guided prediction with AlphaFold (\autoref{ch:alphafold}). Therefore, this section discusses the computation of the symmetry relationship between consecutive monomers, and how it can be applied to generate new configurations of monomers.

In the following, denote by $\mathrm{SO}(3)$ the special orthogonal group of rotation matrices, and write $\mathrm{SE}(3) = \mathrm{SO}(3)\times \mathbb{R}^3$ for the special Euclidean group of rigid motions in three-dimensional space. For a transform $T = (R, \vec{\mathbf{t}}) \in \mathrm{SE(3)}$, the induced motion on a vector $\vec{\mathbf{x}} \in \mathbb{R}^3$ is computed as $T\vec{\mathbf{x}} = R\vec{\mathbf{x}} + \vec{\mathbf{t}}$. Write $T^{-1} = (R^{-1}, -R^{-1}\vec{\mathbf{t}}) = (R^T, -R^T \vec{\mathbf{t}})$ for the inverse transform and $T^n = T\circ ... \circ T$ for the $n$-th power of $T$, where composition of transforms is defined as $T_1\circ T_2 = (R_1R_2, \vec{\mathbf{t_1}} + R_1\vec{\mathbf{t_2}})$. 
...
\begin{equation}
    \vec{\mathbf{r}}_{j,i} = T_0^{-1} \vec{\mathbf{r}}_{j,i}^{\,\text{original}} = \vec{\mathbf{r}}_{j,i}^{\,\text{original}} - \vec{\mathbf{r}}_{A}^{\,\text{original}}
\end{equation}
...
\begin{equation}
T_j = \underset{T \in \mathrm{SE}(3)}{\arg\min} \sum_i \left\| T\vec{\mathbf{r}}_{A,i} - \vec{\mathbf{r}}_{j,i} \right\|^2
\end{equation}
...
\vec{\mathbf{r}}_{j,i}^{\,\text{original}} = (T_0 \circ T_R^j) \vec{\mathbf{r}}_{A,i}
\end{equation}

Using \autoref{eq:01_symmetry}, four different configurations of monomers were generated and used throughout the following sections (\autoref{tab:01_symmetry}). A helical configuration consisting of thirteen consecutive monomers, a 3x3 neighborhood of nine monomers, a trimer consisting of three consecutive monomers, and a pentamer consisting of five monomers arranged in a cross-shape. 

Despite the small standard deviation of $T_R$, the deviation of individual atom positions in the helical thirteen-monomer reconstruction compared to the data from the PDB entry reaches up to $\SI{0.8}{\angstrom}$. This is due to lever effects caused by small deviations in the rotation. The difference in structure introduces no new clashes, but slightly reduces the number of interchain contacts by $\SI{2}{\percent}$, as computed with ChimeraX \cite{ChimeraX2023}.